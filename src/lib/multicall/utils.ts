...
export async function executeMulticall(
  ...
  if (!provider || provider.network?.chainId !== chainId) {
    ...
    staticProvider = new ethers.providers.StaticJsonRpcProvider(rpcUrl, { chainId, name: CHAIN_NAMES_MAP[chainId] });
  }

  const multicall = getMulticallLib(!provider ? staticProvider : provider);
